...
class DictBlock(nn.Module):
    ...
    def fista_forward(self, x):
        ...
        for i in range(self.n_steps):
            ...
            if i == 0:
                ...
                # print(c.shape, weight.shape, x.shape, xp.shape)
                r = x.repeat(1, self.n_dict, 1, 1) - xp
                c = c + step_size * F.conv2d(r, weight, bias=None, stride=self.stride, padding=self.padding)
                c = self.nonlinear(c)
                ...
                xp = F.conv_transpose2d(c, weight, bias=None, stride=self.stride, padding=self.padding,
                                        output_padding=self.conv_transpose_output_padding)
                r = x.repeat(1, self.n_dict, 1, 1) - xp
                c = a + step_size * F.conv2d(r, weight, bias=None, stride=self.stride, padding=self.padding)
                c = self.nonlinear(c)
    ...
    def update_stepsize(self):
        self.step_size = [0.9 / self.power_iteration(self.weight)]

        self.nonlinear.lambd = self.lmbd * self.step_size[0]
        self.nonlinear.scaling_mu = 1.0 / (1.0 + self.mu * self.step_size[0])
...
class DictBlock_pad(DictBlock):

    def conv2d(self, x_in, weight):

        # print(x_in.size())

        x_pad = F.pad(
                    x_in,
                    pad=tuple([self.padding] * 4),
                    mode=self.padding_mode,   # 'constant', 'reflect', 'replicate' or 'circular'
                )

        # print(x_pad.size())
        return F.conv2d(x_pad, weight, bias=None, stride=self.stride)

    def conv_transpose2d(self, x_in, weight):
        ...
        #
        # print(x_pad.size())
        # out = F.conv_transpose2d(x_pad, weight, bias=None, stride=self.stride)
        # print("out size", out.size())
        #
        # out_pad = F.pad(
        #     out,
        #     pad=(0, self.padding, 0, self.padding),
        #     mode=self.padding_mode,
        # )
        # print("out pad size", out_pad.size())
        # return out_pad
        return F.conv_transpose2d(x_in, weight, bias=None, stride=self.stride, padding=self.padding,
                                  output_padding=self.conv_transpose_output_padding)
    ...
    def fista_forward(self, x):
        ...
        for i in range(self.n_steps):
            ...
            if i == 0:
                ...
                c = step_size * self.conv2d(x.repeat(1, self.n_dict, 1, 1), weight)
                # print("c size", c.size())
                c = self.nonlinear(c)
            elif i == 1:
                c_pre = c
                # weight = self.normalize(weight)
                xp = self.conv_transpose2d(c, weight)

                r = x.repeat(1, self.n_dict, 1, 1) - xp
                c = c + step_size * self.conv2d(r, weight)
                c = self.nonlinear(c)
                t = (math.sqrt(5.0) + 1.0) / 2.0
            else:
                t_pre = t
                ...
                xp = self.conv_transpose2d(c, weight)
                r = x.repeat(1, self.n_dict, 1, 1) - xp
                c = a + step_size * self.conv2d(r, weight)
                c = self.nonlinear(c)

            if self.non_negative:
                c = F.relu(c)

            self.closs.append(torch.sum(c ** 2) / c.shape[0])
            self.c_error.append(torch.mean((c - c_pre) ** 2))
            xp = self.conv_transpose2d(c, weight)
            r = x.repeat(1, self.n_dict, 1, 1) - xp
            self.rloss.append(torch.sum(torch.pow(r, 2)) / r.shape[0])
